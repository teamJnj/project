...
<script type="text/javascript">
	$(function() {
		...
		$("#selectPetSort").change(function() {
					...
					location.href = "/jnj/menu/board_find/view_filter?petSort="+$petSort+"&firstAddr="+$firstAddr;
				});
		...
		$.each($findList,function(i, $find) {
			...
			var $a = $("<a></a>").attr("href","/jnj/menu/board_find/view?" + "findNo="+ $find.findNo + "&findDivision="+ $find.findDivision).appendTo($classThumbnail);
			...
			$("<a class='btn btn-block' id='boardFindViewBtn'></a>").attr("href","/jnj/menu/board_find/view?"+ "findNo=" + $find.findNo+ "&findDivision="+ $find.findDivision).text("상세보기").appendTo($findViewBtn);
		})
		...
		$("#write").on("click", function() {
			location.href = "/jnj/menu/board_find/write";
		})
		...
		for (var i = $pagination.startPage; i <= $pagination.endPage; i++) {
			...
			if($firstAddr==null && $petSort==null) 
				li.wrapInner($("<a></a>").attr('href','/jnj/menu/board_find/list?&pageno='+i).css({"background-color" : "#337ab7","border" : "1px solid #337ab7","color" : "white"	}));
			else {
				if ($pagination.pageno == i) 
					li.wrapInner($("<a></a>").attr('href','/jnj/menu/board_find/view_filter?petSort='+$petSort+ '&firstAddr=' + $firstAddr +'&pageno=' + i).css({"background-color" : "#337ab7","border" : "1px solid #337ab7","color" : "white"	}));
				else 
					li.wrapInner($("<a></a>").attr('href',	'/jnj/menu/board_find/view_filter?petSort='+$petSort+ '&firstAddr=' + $firstAddr +'&pageno=' + i));	
			}
		...
		if ($pagination.next > 0) {
			...
			li.wrapInner($("<a></a>").attr('href','/jnj/menu/board_find/list?pageno=' + $pagination.next));
		}
