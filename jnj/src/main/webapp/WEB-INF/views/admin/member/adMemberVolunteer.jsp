...
<head>
	...
	<script>
		...
		$(function(){
			...
			// 모집만 보기
			$("#vRecruit").on("click", function(e){
				e.preventDefault();
				screenState = TYPE_RECURIT;
				$info.colName = "writeDate";
				$info.type = TYPE_RECURIT;
			...
			// 참여만 보기
			$("#vApply").on("click", function(e){
				e.preventDefault();
				screenState = TYPE_APPLY;
				$info.colName = "writeDate";
				$info.type = TYPE_APPLY;
				$info.searchColName = null;
				$info.searchText = null;
				$("#currentPage").text( "봉사 [참여] 내역" );
				sortSelect();
			});
			...
			$("#searchBtn").on( "click", function (){
				...
				$.ajax({
					url 	: "/jnj/admin/member_volunteer/sort",
					type 	: "get", 
			...
			$(".pagination").on("click", "a[class=page-link]", function(e){
				...
				console.log( $info );
				location.href="/jnj/admin/member_volunteer?"+formDataSetting();
			});
			...
			$("#volTable").on("click", ".applyPeople", function(e){
				...
				console.log( $memberVolunteerList[$(this).attr("data-idx")].volunteerNo );
				location.href="/jnj/admin/member_volunteer/apply_list?detailState="+$memberVolunteerList[$(this).attr("data-idx")].detailState+"&volunteerNo="+$memberVolunteerList[$(this).attr("data-idx")].volunteerNo+"&memberId="+$member.memberId;			
			});
			...
			$("#volTable").on("click", ".title", function(e){
				e.preventDefault();
				window.open("/jnj/admin/member_volunteer/view?volunteerNo="+$memberVolunteerList[$(this).attr("data-idx")].volunteerNo+"&memberId="+$member.memberId,
			...
			$("#myModal").on("click", "#yesBtn", function(){
				...
				var strUrl = formDataSetting()+"&volunteerNo="+$memberVolunteerList[$(this).attr("data-idx")].volunteerNo+"&${_csrf.parameterName}=${_csrf.token}";
				console.log(strUrl);
				$.ajax({
					url : "/jnj/admin/member_volunteer/delete",
					type : "post",
					data : strUrl,
					success : function(result){
			...
			$("#myModal").on("click", "#cancleBtn", function(){
				...
				console.log("모달 예쓰! : " + $memberVolunteerList[$(this).attr("data-idx")].volunteerNo );
				
				var form = $("#infoForm");
				form.empty();
				
			    $("<input type='hidden' name='applyId' value='"+ $member.memberId +"' multiple='multiple' >").appendTo(form);
			    $("<input type='hidden' name='volunteerNo' value='"+ $memberVolunteerList[index].volunteerNo +"' multiple='multiple' >").appendTo(form);
		    	...
		    	$.ajax({
					url : "/jnj/admin/member_volunteer/apply/apply_cancle",
					type : "post",
			...
			$("#volCommentBtn").on("click", function(){
				location.href="/jnj/admin/member_volunteer/comment?memberId="+$member.memberId+"&writeId="+$member.memberId;				
			});
			...
			$("#myModal").on("click", "#blockModalBtn", function(){
			    ...
			    $("<input type='hidden' name='volunteerNo' value='"+ $memberVolunteerList[index].volunteerNo +"' >").appendTo(form);
		    	$("<input type='hidden' name='${_csrf.parameterName}' value='${_csrf.token}'>").appendTo(form);
		   
		    	var url = formDataSetting()+"&"+form.serialize();
				
		    	$.ajax({
					url : "/jnj/admin/member_volunteer/block",
					type : "post",
					data : url,
					success : function(result){
						$("#myModal").modal("hide");
			...
			$("#myModal").on("click", "#reportModalBtn", function(){
		    	...
		    	$.ajax({
					url : "/jnj/admin/member_volunteer/report",
					type : "post",
					data : url,
					success : function(result){
						$("#myModal").modal("hide");
			...
			function initValues( map ){
				...
				$memberVolunteerList = map.memberVolunteerList;
				$info = map.adMVolunteerInfo;
				$pagination = map.pagination;
				
				$member = ${member};
				$info.hostId = $member.memberId;
				$info.memberId = $member.memberId;
				
				if( $info.type == TYPE_RECURIT ){
					$totalVolunteerState = map.totalVolunteerState;
					$totalDetailState = map.totalDetailState;
				}
				else
					$totalApplyState = map.totalApplyState;
				
				screenState 	= $info.type;
			...
			function display_setting( state ){
				...
				$.each($memberVolunteerList, function(idx, volunteer) {
					
					table.row.add( [
						"<a class='title' href='#' data-idx='"+idx+"'>"+volunteer.volunteerTitle+"</a>",
						volunteer.hostId,
						(screenState==TYPE_RECURIT)?("<a class='applyPeople' href='#' data-idx='"+idx+"'>"+volunteer.applyPeople+"/"+volunteer.maxPeople+"</a>"):(volunteer.applyPeople+"/"+volunteer.maxPeople),
						volunteer.writeDate,
						currentVolunteerState( volunteer.volunteerState, volunteer.detailState, volunteer.volunteerApplyState ),
						(screenState==TYPE_RECURIT)?volunteer.reportCnt:"-",
				...
				$("#currentPage").text( (screenState==TYPE_RECURIT)? "봉사 [모집] 내역" : "봉사 [참여] 내역");
			}
			...
			function display_totalState(){
                ...
                if( $info.type == 1 ){
	                ...
	                var vasCnt = new Array( 0, 0 );
	                
	    			$.each($totalApplyState, function(idx, vaState) {
	    				
	    				if( vaState.VOLUNTEERAPPLYSTATE == 0 ) 		// 블락
	    					vasCnt[0] = vaState.COUNT;
	    				else
	    					vasCnt[1] = vaState.COUNT;
	    			});
					
					var div = $("<div class='alert alert-success' id='saTotalBtn'></div>").appendTo(totalDiv);
	                $("<span>전체 : </span>").appendTo(div);
	                $("<span></span>").text((vasCnt[0]+vasCnt[1])+"건").appendTo(div);
	                
	                div = $("<div class='alert alert-success' id='saCancleBtn'></div>").appendTo(totalDiv);
	                $("<span>취소(거절) : </span>").appendTo(div);
	                $("<span></span>").text(vasCnt[0]+"건").appendTo(div);
	                
	                div = $("<div class='alert alert-success' id='saApplyBtn'></div>").appendTo(totalDiv);
	                $("<span>신청 : </span>").appendTo(div);
	                $("<span></span>").text(vasCnt[1]+"건").appendTo(div);
			...
			function btnState( volunteer, detailState, idx ){
				...
				// 봉사 참여 내역 볼 때
					...
					if( volunteer.volunteerApplyState == 1 ){ // case
						if( detailState == 1 ) btnStr = "<a id='joinBtn' href='#' data-idx='"+idx+"'>참여 취소</a>";
						else if( detailState == 2 ) btnStr = "-";
						else if( detailState == 3 ) btnStr = "-";
						else if( detailState == 4 ) btnStr = "-";
					}
					else
						btnStr = "-";
				}
			...
			function currentVolunteerState( volunteerState, detailState, volunteerApplyState ){
				...
				// 봉사 모집내역을 보는 상태 일 때
				if( (volunteerApplyState == null || volunteerApplyState == "") && $info.type == TYPE_RECURIT){
				...
				// 봉사 참여내역을 보는 상태 일 때
					...
					if( volunteerApplyState == 1 ){ // case
						if( detailState == 1 ) 		strState = "신청(모집중)";
						else if( detailState == 2 ) strState = "신청(모집완료)";
						else if( detailState == 3 ) strState = "봉사완료";
						else if( detailState == 4 ) strState = "인원미달";
					}
					else
			...
			function display_Search(){
				...
				if( select != "volunteerState" && $info.searchColName == "volunteerState" )
					$info.searchText = "%";
				...
				if( select == "volunteerTitle" || select == "hostId" ){
					...
					console.log("뭔데 값이? : " + $info.searchText);
					...
					if( $info.type == 1){
						...
						$("<option value='2'>전체</option>").appendTo( selectbox );
						$("<option value='0'>취소(거절)</option>").appendTo( selectbox );
						$("<option value='1'>신청</option>").appendTo( selectbox );
						
						$("#selectedOption").val(($info.searchText == "%")?"2":$info.searchText).attr("selected", "selected");
					}
				}
				else{
					selectTextDiv.hide();
			...
			function formDataSetting(){
				...
				if( $info.memberId != null ) $("<input type='hidden' name='hostId' value='"+$info.hostId+"' >").appendTo( form );
					...
					<div id="vvnav" style="background-color:white; overflow:hidden;">
						...
						<ol id="sortVol" class="breadcrumb" style="background-color:white; float:right">
							<li class="breadcrumb-item">
								<a id="vRecruit" href="#">모집내역</a>
							</li>
							<li class="breadcrumb-item">
								<a id="vApply" href="#">참여내역</a>
							</li>
						</ol> 
			    	</div>
