...
<head>
	...
	<script>
		...
		$(function(){
			...
			// 댓글내용순 정렬
			$("#sponsorReply").on("click", function(e){
				e.preventDefault();
				$info.colName = "sponsorReply";
				sortSelect();
			});
			...
			$("#searchBtn").on( "click", function (){
				...
				if( selectbox == "centerName" || selectbox == "petName" || selectbox == "sponsorReply" ){
				...
				$.ajax({
					url 	: "/jnj/admin/member_sponsor/comment/sort",
					type 	: "get", 
			...
			$(".pagination").on("click", "a[class=page-link]", function(e){
				...
				location.href="/jnj/admin/member_sponsor/comment?"+formDataSetting();
			});
			
			$("#sponsorRecordBtn").on("click", function(){
			...
			function display_Search(){
				...
				if( select == "centerName" || select == "petName" || select == "sponsorReply" ){
					selectTextDiv.show();
					selectMoneyDiv.hide();
					selectDateDiv.hide();
					selectboxDiv.hide();
...
<body>
    ...
    <div id="scorpTotal">
    	<div id="tableTotal">
    		...
    		<div id="menuDiv">
    			<button type="button" class="btn btn-warning" id="sponsorRecordBtn" >후원내역</button>
    			<button type="button" class="btn btn-warning" id="sponsorCommentBtn" disabled="disabled">후원댓글</button>
    		</div>
    		
			<div class="card mb-3">
				<div class="card-header"> <i class="fa fa-table"></i> 후원댓글내역 </div>
				<div class="card-body">
					...
					<table id="sponsorTable" class="table display" cellspacing="0" width="100%">
						<thead>
					    	<tr>
					        	<th style="width:110px;">센터명</th>
					            <th style="width:50px;">분류</th>
					            <th style="width:80px;">이름</th>
					            <th style="width:80px;">후원금</th>
					            <th>내용</th>
		...
		<div id="searchSelectDiv">
			<div id="selectDiv">
				<select id="mSelect" class="form-control"> 
					...
					<option value="sponsorReply">내용</option> 
