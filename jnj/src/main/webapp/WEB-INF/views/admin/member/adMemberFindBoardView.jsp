...
<script type="text/javascript">
	$(function() {
		...
		// 버튼을 클릭을 했다면
		if ($find.reportCnt == 3 || $find.findState == 0) {
			$("table").hide();
			$("#findNo").hide();
			$("#findContent").hide();
			$("#findDate").hide();
		...
		$("<img class='img-responsive' style=\"width:320px; height:200px;\"></img>").attr("src", "/jnjimg/find/" + $find.petImg).appendTo($table);
		...
		if ($userName == $find.writeId) {
			...
			$("<button type='button'></button>").attr("id", "findBoardReport").attr("name", "findBoardReport").text("신고").appendTo("#table2");
		...
		function printReply($findComment) {
			...
			$.each($findComment, function(i, $findComment) {
				...
				$("<td class='td'></td>").text($findComment.writeId).appendTo(
						$tr);
				$("<td class='tdContent'></td>").text(
						$findComment.commentContent).appendTo($tr);
				$("<td class='td'></td>").text($findComment.writeDate)
						.appendTo($tr);
				...
				if ($userName == $findComment.writeId) {
					$("<button type='button' data-idx='"+i+"'></button>").attr(
							"class", "updateFindComment").attr(
							"data-FindCommentNo", $findComment.findCommentNo)
							.text("수정").css("display", "block").appendTo(td);
					$("<button type='button'></button>").attr("class",
							"deleteFindComment").attr("data-findCommentNo",
							$findComment.findCommentNo).attr("data-findNo",
							$find.findNo).text("삭제").css("display", "block")
							.appendTo(td);

					$("<button type='button' data-idx='"+i+"'></button>").attr(
							"class", "updateCommentContent").attr(
							"data-FindCommentNo", $findComment.findCommentNo)
							.text("수정완료").css("display", "none").appendTo(td);
					$("<button type='button'></button>")
							.attr("class", "cancel").attr("data-findCommentNo",
									$findComment.findCommentNo).text("수정취소")
		...
		$(".deleteFindComment").on("click", function() {
					if (confirm("정말 삭제하시겠습니까??") == true) { //확인
						...
						formData.append("findCommentNo", $(this).attr(
								"data-findCommentNo"));
						formData.append("_csrf", "${_csrf.token}");
						console.log(formData);
		...
		$("#reply").on("click", ".updateFindComment", function() {
							...
							var td = $(this).parent().parent().children("td")
									.eq(1);
							var str = $(this).parent().parent().children("td")
									.eq(1).text();

							$(this).parent().parent().children("td").eq(1)
									.text("");

							$(
									"<textarea rows='4' cols='120' style='width: 400px' id='commentContent' name='commentContent'></textarea>")
									.val(str).appendTo(td);

							var td1 = $(this).parent().children("button").eq(0)
									.css("display", "none");
							var td2 = $(this).parent().children("button").eq(1)
									.css("display", "none");
							var td3 = $(this).parent().children("button").eq(2)
									.css("display", "block");
							var td4 = $(this).parent().children("button").eq(3)
									.css("display", "block");
		...
		$("#findBoardDelete").on("click", function() {
					if (confirm("정말 삭제하시겠습니까??") == true) {
						...
						$("<input>").attr("type", "hidden").attr("name",
								"findNo").val($find.findNo).appendTo($form);
						$("<input>").attr("type", "hidden").attr("name",
		...
		$("#findBoardReport").on("click", function() {
			...
			formData.append("findNo", $find.findNo);
			formData.append("findState", $find.findState);
			formData.append("_csrf", "${_csrf.token}");
			$.ajax({
				url : "/jnj/menu/board_find/report",
				type : "post",
				data : formData,
				processData : false, // FormData 전송에 필요한 설정
				contentType : false, // FormData 전송에 필요한 설정
		...
		$("#findBoardUpdate").on("click", function() {
			location.href = "/jnj/menu/board_find/update?findNo=" + $find.findNo;
		});
...
<body>
<h2 id="caca">- 찾아요</h2>
	<a id="backList" href="/jnj/menu/board_find/list">목록으로</a>
	<br>
	<hr>
<form>
...
<div class="container" id="container">
  ...
  <table class="table">
  	<thead>
		...
		<tr>
			<th id="contentTh">잃어버린 장소</th>
			<td class="contentTd" id="findAddr"></td>
		</tr>
		<tr><td colspan="2"><div id="map" style="width:600px; height:350px; margin-left: 200px; margin-top: 30px; margin-bottom: 60px;"></div></td><td></td></tr>
	</thead>
  </table>
</div>
