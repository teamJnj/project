	...
	<script>
		...
		$(function(){
			...
			$("#completBtn").on("click", function(e){
				$info.pageno = 1;
				searchState( "marketApplyState", "2");
			});
			$("#failBtn").on("click", function(e){
				$info.pageno = 1;
				searchState( "marketApplyState", "3");
			});
			...
			$("#searchBtn").on( "click", function (){
				...
				$.ajax({
					url 	: "/jnj/admin/member_market/sort",
					type 	: "get", 
			...
			$("#myModal").on("click", "#cancleBtn", function(){
		    	...
		    	$.ajax({
					url : "/jnj/admin/member_market/apply/apply_cancle",
					type : "post",
			...
			$(".pagination").on("click", "a[class=page-link]", function(e){
				...
				location.href="/jnj/admin/member_market?"+formDataSetting();
			});
			...
			$("#marketTable").on("click", ".detailClick", function(e){
				...
				window.open("/jnj/admin/member_market/view?marketNo="+$memberMarketList[$(this).attr("data-idx")].marketNo+"&memberId="+$member.memberId,
					"관리자 - 마켓 상세", "'width="+$( window ).width()+", height="+$( window ).height()+"'"); 
			});
			
			
			// 마켓 댓글 클릭 시
			$("#marketCommentBtn").on("click", function(){
				location.href="/jnj/admin/member_market/comment?memberId="+$member.memberId+"&writeId="+$member.memberId;				
			});
			...
			function display_setting( state ){
				...
				$.each($memberMarketList, function(idx, ml) {
					...
					table.row.add( [
						...
						ml.marketDate,
						ml.boothNum+"개",
						numberWithCommas(ml.payMoney)+"원",
						ml.applyDate,
						currentApplyState(ml.marketApplyState),
						(ml.marketApplyState==MARKETAPPLY_MARKETAPPLYSTATE_APPLY)?"<a href='#' id='cancleBtn' data-idx='"+idx+"' >취소</a>":"-"
			        ]).draw(false);
				...
				$.each($state, function(idx, state) {
					...
					else if( state.MARKETAPPLYSTATE == 1 )
						$("#apply").text(state.COUNT + "건");
					else if( state.MARKETAPPLYSTATE == 3 )
						$("#complet").text(state.COUNT + "건");
					else
						$("#fail").text(state.COUNT + "건");
			...
			// 펫 상태 표기 변환
			function currentApplyState( marketApplyState ){
				
				var MARKETAPPLY_MARKETAPPLYSTATE_CANCLE = 0;
				var MARKETAPPLY_MARKETAPPLYSTATE_APPLY = 1;
				var MARKETAPPLY_MARKETAPPLYSTATE_COMPLETE = 2;
				var MARKETAPPLY_MARKETAPPLYSTATE_LACK = 3;
				
				var strState = "취소(거절)";
				if( marketApplyState == MARKETAPPLY_MARKETAPPLYSTATE_APPLY )
					strState = "신청완료";
				else if( marketApplyState == MARKETAPPLY_MARKETAPPLYSTATE_COMPLETE )
					strState = "완료";
				else if( marketApplyState == MARKETAPPLY_MARKETAPPLYSTATE_LACK )
					strState = "인원미달";
...
<body>
    ...
    <div id="scorpTotal">
    	...
    	<div id="tableTotal">
    		...
    		<div id="menuDiv">
    			<button type="button" class="btn btn-warning" id="marketRecordBtn" disabled="disabled">마켓내역</button>
    			<button type="button" class="btn btn-warning" id="marketCommentBtn">마켓댓글</button>
    		</div>
    		
			<div class="card mb-3">
				<div class="card-header"> <i class="fa fa-table"></i> 프리마켓 신청 내역 </div>
				<div class="card-body">
					...
					<div id="totlaNumDiv">
	                    ...
	                    <div class="alert alert-success" id="failBtn">
	                    	<span>인원미달 : </span>
	                    	<span id="fail">0건</span>
	                    </div>
	                    
	                    <div class="alert alert-success" id="cancleBtn">
	                    	<span>취소(거절) : </span>
					...
					<div id="ssnav" style="background-color:white; overflow:hidden;">
						<ol id="sortMarket" class="breadcrumb" style="background-color:white; float:right">
							...
							<li class="breadcrumb-item">
								<a id="sTitle" href="#">제목순</a>
							</li>
							<li class="breadcrumb-item">
								<a id="sBooth" href="#">부수별</a>
							</li>
							<li class="breadcrumb-item">
								<a id="sMoney" href="#">참가비순</a>
							</li>
							<li class="breadcrumb-item">
								<a id="sState" href="#">상태별</a>
							</li>
							<li class="breadcrumb-item">
								<a id="sOpen" href="#">개최날짜순</a>
							</li>
							<li class="breadcrumb-item">
								<a id="sApply" href="#">신청날짜순</a>
							</li>
						</ol>
			    	</div>
					...
					<table id="marketTable" class="table display" cellspacing="0" width="100%">
						<thead>
					    	<tr>
					            ...
					            <th style="width:50px;" >부수</th>
					            <th style="width:80px;" >참가비</th>
		...
		<div id="searchSelectDiv">
			<div id="selectDiv">
				<select id="mSelect" class="form-control"> 
					...
					<option value="boothNum">부수</option> 
					<option value="payMoney">참가비</option> 
