...
<head>
	...
	<script>
		...
		$(function(){
			...
			// 누적 후원금 순 정렬
			$("#aSponsor").on("click", function(e){
				e.preventDefault();
				$info.colName = "sponsorMoney";
				sortSelect();
			});
			...
			$("#searchBtn").on( "click", function (){
				...
				$.ajax({
					url 	: "/jnj/admin/center_sponsor/sort",
					type 	: "get", 
			...
			$("#petTable").on("click", ".detaileBtn", function(){
				console.log("상세뷰 클릭 : "+$center.centerId);
				window.open("/jnj/admin/center_sponsor/detail?centerId="+$center.centerId+"&petNo="+$centerSponsorList[$(this).attr("data-idx")].petNo, 
			...
			function display_setting( state ){
				...
				$.each($centerSponsorList, function(idx, pet) {
					table.row.add( [
						...
						pet.mercyDate,
						pet.writeDate,
						numberWithCommas(pet.sponsorMoney)+"원",
						"<a href='#' class='detaileBtn' data-idx='"+idx+"'>상세보기</a>"
			        ]).draw(false);
					
				});
				
				$("#total").text( numberWithCommas($totalSponsorState) + "원");
		...
		<div id="searchSelectDiv">
			<div id="selectDiv">
				<select id="mSelect" class="form-control"> 
					...
					<option value="sponsorMoney">누적후원금</option> 
				</select>
			</div>
			<div id="searchDiv">
