	...
	<script>
		...
		$(function(){
			...
			$("#searchBtn").on( "click", function (e){
				...
				$.ajax({
					url 	: "/jnj/admin/center_adopt/sort",
					type 	: "get", 
			...
			// 상세보기 클릭 시
			$("#petTable").on("click", ".detaileBtn", function(e){
				e.preventDefault();
				console.log("상세뷰 클릭");
				window.open("/jnj/admin/center_adopt/detail?petNo="+$centerAdoptList[$(this).attr("data-idx")].petNo, 
						"관리자 - 입양 상세", "'width="+$( window ).width()+", height="+$( window ).height()+"'"); 
			});
			...
			function initValues( map  ){
				...
				$map = map;
				$centerAdoptList = $map.centerAdoptList;
				$info = $map.adCAdoptInfo;
			...
			function display_setting( state ){
				...
				$.each($centerAdoptList, function(idx, pet) {
					table.row.add( [
				...
				$.each( $state, function(idx, state){
					...
					if( state.PETSTATE == 1 ) 		$("#standby").text( state.COUNT + "건" );
					else if( state.PETSTATE == 2 ) $("#receipt").text( state.COUNT + "건" );
					else if( state.PETSTATE == 3 ) $("#process").text( state.COUNT + "건" );
					else if( state.PETSTATE == 4 ) $("#adopt").text( state.COUNT + "건" );
					else 							$("#etc").text( state.COUNT + "건" );
			...
			function currentMemberAdoptState( petNo, petState, idx ){
				...
				var strState = "취소";
				createBtn = "<a href='#' id='detaileBtn' data-idx='"+idx+"' data-pet='"+petNo+"'>상세보기</a>"
									+"<br>"
