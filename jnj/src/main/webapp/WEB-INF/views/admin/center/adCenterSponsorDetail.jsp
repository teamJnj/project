...
<head>
	...
	<script>
		...
		$(function(){
			...
			var tempMap = ${map};
			console.log(tempMap);
			var $centerSponsorList;
			var $info;
			var $pagination;
			var $center;
			var $totalSponsorState;
			...
			$("#asponsorNo").on("click", function(e){
				e.preventDefault();
				$info.colName = "sponsorNo";
				sortSelect();
			});
			
			$("#aSponsorMoeny").on("click", function(e){
				e.preventDefault();
				$info.colName = "sponsorMoeny";
				sortSelect();
			});
			
			$("#aachieveDate").on("click", function(e){
				e.preventDefault();
				$info.colName = "achieveDate";
				sortSelect();
			});
			...
			$("#petTable").on("click", ".detaileBtn", function(){
				console.log("상세뷰 클릭");
				window.open("/jnj/admin/center_sponsor/detail/detail?petNo="+$centerSponsorList[$(this).attr("data-idx")].petNo, 
			...
			function initValues( map  ){
				...
				$centerSponsorList = map.centerSponsorList;
				$info = map.adCSponsorDetailInfo;
				...
				$totalSponsorState	= map.totalSponsorState;
			...
			function display_setting( state ){
				...
				$.each($centerSponsorList, function(idx, pet) {
					table.row.add( [
						pet.sponsorNo,
						pet.goalMoney,
						pet.sponsorMoney,
						(pet.achieveDate==null)?"-":pet.achieveDate,
						"-"
			        ]).draw(false);
			...
			function formDataSetting(){
				...
				if( $info.petNo != null ) $("<input type='hidden' name='petNo' value='"+$info.petNo+"' >").appendTo( form );
				...
				<div class="card-body">
					...
					<div id="aanav" style="background-color:white; overflow:hidden;">
						<ol id="sortAdopt" class="breadcrumb" style="background-color:white; float:right">
							...
							<li class="breadcrumb-item">
								<a id="asponsorNo" href="#">후원번호순</a>
							</li>
							<li class="breadcrumb-item">
								<a id="aSponsorMoeny" href="#">후원금액순</a>
							</li>
							<li class="breadcrumb-item">
								<a id="aachieveDate" href="#">달성날짜순</a>
							</li>
						</ol>
			    	</div>
		...
		<div id="searchSelectDiv">
			<div id="selectDiv">
				<select id="mSelect" class="form-control"> 
					<option value="sponsorNo">후원번호</option> 
					<option value="sponsorMoney">후원금액</option> 
					<option value=achieveDate>달성날짜</option> 
				</select>
			</div>
			<div id="searchDiv">
