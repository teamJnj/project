	...
	<script>
		...
		$(function(){
			...
			$("#myModal").on("click", "#cancleBtn", function(){
		    	...
		    	$.ajax({
					url : "/jnj/admin/orders/apply/apply_cancle",
					type : "post",
			...
			$(".pagination").on("click", "a[class=page-link]", function(e){
				...
				location.href="/jnj/admin/orders?"+formDataSetting();
			});
			...
			function display_Search(){
				...
				if( select == "orderNo" || select == "orderId" ){
					selectTextDiv.show();
					selectMoneyDiv.hide();
					selectDateDiv.hide();
					selectboxDiv.hide();
			...
			function currentBtn( orderState, idx ){
				var str = "<a href='#' class='standbyClick' data-idx='"+idx+"'>배송준비</a>";
