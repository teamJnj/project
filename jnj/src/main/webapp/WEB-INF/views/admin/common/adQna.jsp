	...
	<script>
		...
		$(function(){
			...
			$(".pagination").on("click", "a[class=page-link]", function(e){
				...
				location.href="/jnj/admin/qna?"+formDataSetting();
			});
			
			
			// 상세보기 클릭 시
			$("#qnaTable").on("click", ".titleClick", function(){
				window.open("/jnj/admin/qna/view?qnaNo="+$qnaList[$(this).attr("data-idx")].qnaNo+"&writeId="+$qnaList[$(this).attr("data-idx")].writeId, 
						"관리자 - qna 상세", "'width="+$( window ).width()+", height="+$( window ).height()+"'"); 
			});
			...
			function initValues( map ){
				...
				$state1 = map.stateCount3;
				$state2 = map.stateCount4;
			...
			function display_setting( state ){
				...
				$.each( $state2, function(idx, state){
					if( state.ANSWERCONTENT != null || state.ANSWERCONTENT != "")
						$("#complet").text( (comCnt+=state.COUNT)+"건");
					totalCnt += state.COUNT;
				});
			...
			function display_Search(){
				...
				else if( select == "qnaDivision" ){
					selectTextDiv.hide();
					selectDateDiv.hide();
					selectboxDiv.show();
					
					$("<option value=''>전체</option>").appendTo( selectbox );
					$("<option value='1'>일반회원</option>").appendTo( selectbox );
					$("<option value='2'>센터회원</option>").appendTo( selectbox );
		...
		<div id="searchSelectDiv">
			<div id="selectDiv">
				<select id="mSelect" class="form-control"> 
					...
					<option value="qnaDivision">회원</option> 
