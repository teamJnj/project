...
$(function() {
	...
	$("<td></td>").text(orderViewPay[0].PAYMONEY).appendTo(payMoneytr);
	var payMoneytr2 = $("<tr></tr>").appendTo(orderview2);
	$("<th></th>").text("총 결제금액").appendTo(payMoneytr2);
	$("<td></td>").text(orderViewPay[0].PAYMONEY).appendTo(payMoneytr2);
	...
	if(orderViewPay[0].PAYWAY==1) {
		$("<span></span>").text("카드").appendTo(paywaytd);
	...
	$("<span></span>").text("111-111111-1111111").appendTo(paywaytd);
	...
	$.each(orderViewOrders, function(idx, orderView) {
		...
		$("<td></td>").attr("rowspan", 2).text(orderView.orderViewOrders.MONEY+"원").appendTo(goodstr);
		var statetd = $("<td></td>").appendTo(goodstr);
		var state = orderView.orderViewOrders.ORDERRECORDSTATE;
		if(state==0) {
			$("<span></span>").text("주문취소").appendTo(statetd);
		} else if(state==1) {
			$("<span></span>").text("주문완료").appendTo(statetd);
		} else if(state==2) {
			$("<span></span>").text("입금완료").appendTo(statetd);
		} else if(state==3) {
			$("<span></span>").text("배송준비중").appendTo(statetd);
		} else if(state==4) {
			$("<span></span>").text("배송준비중").appendTo(statetd);
		} else if(state==5) {
			$("<span></span>").text("배송완료").appendTo(statetd);
		} else if(state==6) {
			$("<span></span>").text("구매확정").appendTo(statetd);
		} else if(state==7) {
			$("<span></span>").text("교환/환불중").appendTo(statetd);
		} else if(state==8) {
			$("<span></span>").text("교환/환불완료").appendTo(statetd);
		} 
	...
	$("<span></span>").text(orderViewPay[0].PAYMONEY+"원").appendTo(totoaltd);
	...
	$(".review").on("click", function() {
		...
		var $goodsNo = $no.split('/')[0];
		var $optionNo = $no.split('/')[1];
		var $form = $("<form></form>").attr("target", "리뷰작성").attr("action", "/jnj/menu/store/comment/1").attr("method", "post");
	...
	$(document).on("click", "#rechange", function() {
		...
		for(var i=0; i<orderViewOrders.length; i++) {
			if(orderViewOrders[i].orderViewOrders.ORDERRECORDSTATE==6) {
				cnt++;
			} 
		}
